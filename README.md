...
表格内容会在内部整理成 `Parameter / Symbol / Value / Units` 行，再生成独立表格变化记录。
...
- `--unchanged-similarity 0.985`：只对高相似度、无数字/标识符变化的安全词尾屈折（如 `support/supports`）生效；否定前缀、数值、比较符和技术标识符变化始终保留。
...
- 大小写、普通空格、逗号句号、PDF 抽取出来的 `10 6`/`106`、`125. 0`/`125.0`、箭头样式等显示噪声会尽量忽略；但 `<=`、`>=`、`<`、`>`、`=`、`+0/-2 mV`、`1.0 ps`/`10 ps` 这类可能改变协议含义的符号和数值会保留比较。
...
- 表格 OCR 需要系统额外安装 Tesseract；未安装时不会静默伪装 OCR 结果，报告仍保留截图和结构化表格行。
...
- 调整章节匹配阈值、未变化阈值、每章展示片段数；未变化阈值不会隐藏数值或技术标识符变化。
- 点击 `填入 Demo 文件` 后先把内置 demo PDF 填到输入框，再按需要填写页码并手动开始比较。
