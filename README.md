...
表格内容会尽量按行追加为 `表格行: ...` 片段，例如 `Parameter=... | Symbol=... | Value=... | Units=...`。
这样参数表里的数值变化不会只埋在整页抽取文本里，更适合直接搜索参数名或导出 CSV 复核。
...
- 表格会优先用 `pdfplumber` 版面坐标和表格线提取成行级片段，并在 HTML 中展示旧/新表格截图；如果 PDF 表格没有清晰网格，仍可能退回为普通文本顺序。
...
- 点击 `开始比较 / 生成报告` 后生成 HTML、TXT、CSV 和 JSON 报告。
- 调整章节匹配阈值、未变化阈值、每章展示片段数。
