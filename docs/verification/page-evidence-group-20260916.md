...
报告先按旧版/新版物理页建立连通的 `page-evidence-group`，再把该页涉及的表格、正文和单侧证据卡放入同一组。页面组按页序输出，表格优先、正文随后；同组中的每侧原页截图通过 `data-source-alias` 只保留一份，其他 occurrence 保留可回源链接。表格不再因为配对相似度 `1.000` 被移入折叠附录。正文折叠也不再使用三位小数显示值：只有完整正文内容分数和章节配对分数都精确为 `1.0`，且没有数字、技术标识或语义运算符变化时才允许折叠；长正文中的真实数值变化必须留在主区。读者投影还会过滤运行页脚的版权/草稿声明、选定页窗的内部范围占位和 OCR 重复科学计数法符号；原始 JSON/CSV 继续保留审计事实。
