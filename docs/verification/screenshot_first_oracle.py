...
def check(case, observed):
    ...
    if kind=='partition':
        ...
        assert observed['raw']==1,observed
    elif kind=='short':
...
def main():
    samples=[({'kind':'partition','main':0,'appendix':1},dict(main=0,appendix=1,raw=1,reviewable_text='12 mV')),
    ...
    for p in sys.argv[1:]:
        for c in json.load(open(p))['cases']:assert c['kind'] in {'partition','short','table','invalid'}
