...
`family`。只有实际执行并产出指标、且两侧源 SHA 不同的版本对才计入覆盖数；同一 old/new
字节对即使重复填写不同 family 也只贡献一次，family 会先折叠内部空白并忽略大小写。缺文件
的 skip 和 self-diff 不能凑足门槛。family 名是经审阅 manifest 的受信标签，不是工具从 PDF
自动证明的出版方身份。默认门槛为 `1`，用于普通准确率回归；凡是要宣称“跨标准/跨文档族
通用性”，必须显式设为至少 `2`，并至少包含一组非 OIF 的真实 old/new 人工完整标注版本对。
summary 仅输出 required/executed/complete 数量，不复制 family 名称或输入哈希。
...
- 表格参数、表题、续页和行级值；
- 章节新增、删除、重编号和跨页移动；
- 相同运行页眉不形成噪声，而页眉中的版本号、单位和技术标识变化必须保留；另覆盖页脚、水印、行号和版式噪声。
