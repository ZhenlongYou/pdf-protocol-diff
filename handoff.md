...
- 第一版父前缀中和被两名 reviewer 独立否决：`Protocol Version/Firmware ID/Register identifier 31...` 会被误当子条款。最终实现增加标题结构正向证据：编号后的正文必须同时复用已配对父条款标题至少两个实质词，才允许中和；上述三类技术 ID 已转为 RED/GREEN 回归并保持五面可见。
- 该修复的目标 RED 已在公开 `compare_extractions + write_reports` 路径复现，修复 reviewer blocker 后相邻引用/单位/标识符矩阵 `154/154 PASS`、最终全量 `1036/1036 PASS`（`461.197s`），`compileall` 与 `git diff --check` 通过。
- 真实 OIF 532 复审报告：`/Users/mac/Desktop/test/PDF对比工具_引用差异修复复审_20260813/532/protocol_diff_20260813_220502/protocol_diff_report.html`。浏览器可见文本中两条纯引用旧/新句均为 `0` 次，`Sinusoidal Interface TP4a` 为 `1` 次；JSON 审计仍含旧/新四个引用文本。整体计数保持 `35` 正文、`12` 表格、`7` 公式且结论仍为 `需人工复核`。
...
- 引用差异修复 OIF HTML：`/Users/mac/Desktop/test/PDF对比工具_引用差异修复复审_20260813/532/protocol_diff_20260813_220502/protocol_diff_report.html`
