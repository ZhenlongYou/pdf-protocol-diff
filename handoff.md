...
- 项目虚拟环境中的当前完整测试为 1171/1171 通过，耗时 485.782 秒；慢项来自真实 OIF/协议 PDF 的重复抽取、跨页表格与截图回归，公式禁比的针对性测试为毫秒级。后续若优化测试时间，应缓存同一 PDF 的抽取快照，不能缩减真实语料门禁。
- GUI 真实入口冒烟通过；受控解析基准 5/5 通过。系统 Python 的一次非权威尝试因为缺少 `pymupdf` 依赖失败，不能算产品失败，也不作为最终通过证据；正式结果以项目 `.venv` 日志为准。
- 初次独立审核发现 HTML 折叠文字、Markdown 和 TXT 仍发布 C9/C27/C35 的 Figure/Table 标签残片；不能因联系表可读就通过。修复后的真实 OIF 诊断报告已确认六个残片哨兵在三种读者格式中均为 0。最终交付仍需对新提交重新生成全部联系表，并让三个独立 agent 复审同一份成品。
