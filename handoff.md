...
- oif_entrypoint: `.venv/bin/python main.py --old-pdf /Users/mac/Documents/文件对比工具/oif2024.532.04.pdf --new-pdf /Users/mac/Documents/文件对比工具/oif2024.532.05.pdf --layout-backend native --output-dir /Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/532-final-v8`
- non_oif_entrypoint: `.venv/bin/python main.py --old-pdf '/Users/mac/Documents/New project/work/word_render_v1/PCIe_technical_report_word_v1_20260704.pdf' --new-pdf '/Users/mac/Documents/New project/work/word_render_v2/PCIe_technical_report_word_v2_20260704.pdf' --layout-backend native --output-dir /Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/pcie-final-v8`
- accepted_oif_report: `/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/532-final-v8/protocol_diff_20260811_031422/protocol_diff_report.html`
- accepted_non_oif_report: `/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/pcie-final-v8/protocol_diff_20260811_031424/protocol_diff_report.html`
...
- 普通编号章节下的完整叙述句列表以“已有编号父章节、严格连续、句子形态明确、相邻后项证明”作为结构证据；有界前瞻覆盖当前页和后续两页，普通正文立即终止证明，单条跳号、保留章号及 Part/Annex 编号重启均失败可见。下一真章节后的坐标表格行属于该章正文，不能被前瞻跨过；其他追加表格行也不会伪装成列表间正文。比例/路径使首项不成为标题时，该首项仍可为紧邻后项提供语法证据。规则不使用 OIF、PCIe 或厂商关键词。
- 位于顶部窄区域且在绝大多数选定页重复的运行页眉从正文流分离，但原文作为独立比较单元保留；每个页眉页都相同的固定家具只保留一次，避免页数变化制造差异，同一文档存在多个精确值时则保留逐页次数，`ALPHA→BETA`、`ALPHA→alpha` 或其跨页分布变化都会进入五种报告输出。
...
- 全量：`PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m unittest discover -s tests`，`894` 项通过、`0` 失败，耗时 `326.797s`。
...
- 最终 OIF 532：`35` 张原始正文审计卡、`12` 张原始表格审计卡、`7` 项公式证据、`0` 张视觉变化卡；视觉覆盖保持 incomplete 并阻止 all-clear。v8 的 HTML/Markdown/TXT 隐藏截图中的 Table/Section 纯引用原句，JSON 保留原始事实，`53.125 GHz`、`CMIS-LT`、`MCB` 均可见；固定页眉不因 36/35 页数差制造新卡。
...
- 先前同一渲染器的 v3 HTML 已在应用内浏览器实测首屏表格前置、OIF h2 顺序为表格/公式/正文且 PCIe 人工复核提示可见。v8 再核对 HTML h2、Markdown 章节顺序、三种读者文字和 JSON；应用内浏览器安全策略拒绝直接打开新的本地 `file://`，未绕过该限制，也不把静态检查冒充新的视觉验收。
...
- OIF HTML：`/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/532-final-v8/protocol_diff_20260811_031422/protocol_diff_report.html`
- 非 OIF HTML：`/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/pcie-final-v8/protocol_diff_20260811_031424/protocol_diff_report.html`
- Gold summary：`/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/gold_accuracy_final_v3.json`
- 受控解析 summary：`/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/controlled_parsing_benchmark_final_v3.json`
- 混合 Corpus summary：`/Users/mac/Desktop/PDF对比工具_通用性增强验收_20260811/corpus_summary_final_v3.json`
