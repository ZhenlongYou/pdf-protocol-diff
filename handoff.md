...
- 当前代码的项目虚拟环境完整测试为 1182/1182 通过，耗时 484.035 秒；追加下标与 Table 后缀修复后的重点测试为 378/378 通过。慢项来自真实 OIF/协议 PDF 的重复抽取、跨页表格与截图回归；后续若优化测试时间，应缓存同一 PDF 的抽取快照，不能缩减真实语料门禁。
- 当前提交候选的 GUI 真实入口冒烟通过；受控解析基准 5/5 通过。系统 Python 的一次非权威尝试因为缺少 `pymupdf` 依赖失败，不能算产品失败，也不作为最终通过证据；正式结果以项目 `.venv` 日志为准。
- 初次独立审核发现 HTML 折叠文字、Markdown 和 TXT 仍发布 C9/C27/C30/C34/C35/C36 的 Figure/Table 标签残片；不能因联系表可读就通过。修复后的真实 OIF 诊断报告 `protocol_diff_20260827_020531` 已确认这些旧失败模式在三种读者格式中均消失，并用完整句首正向断言防止过度清理。最终交付仍需对新提交重新生成全部联系表，并让三个独立 agent 复审同一份成品。
