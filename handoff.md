...
- 本轮新增同页表格/正文共享截图与同页表格重复截图回归；定向套件 247 项通过，编译、`git diff --check` 和真实报告结构/图片解码检查通过。完整 `unittest discover -s tests -q` 为 1746 项，11 个失败均为既有基线（7 个旧标点断言、058 表格编号上下文 1 个、532 样例 3 个），另 1 项条件跳过；没有新增失败。
- 本轮代码与测试待提交到持久分支 `project/pdf-protocol-diff`；上一已推送基线仍为 `d70db77f2f830ebb3e6ebd1d454e869c0322c15d`。真实报告只写入 Documents 输出目录，旧报告保持不变。
