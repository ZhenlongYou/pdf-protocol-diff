...
class ReaderAccuracyRegressionTests(unittest.TestCase):
    ...
    def test_body_pua_and_unicode_without_font_provenance_are_not_silently_equal(self) -> None:
        ...
        self.assertIn(
            "<strong>0</strong><span>正文差异候选（章节）</span>",
            reader_outputs[0],
        )
        self.assertIn(
            "<strong>1</strong><span>正文待核实项（未分类）</span>",
            reader_outputs[0],
        )
        ...
        self.assertIn("<strong>0</strong><span>正文差异候选（章节）</span>", operator_html)
        self.assertIn("<strong>1</strong><span>正文待核实项（未分类）</span>", operator_html)
        self.assertIn("U+F03C", operator_html)
