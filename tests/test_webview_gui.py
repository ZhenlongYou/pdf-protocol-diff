...
class WebviewGuiTests(unittest.TestCase):
    ...
    def test_windows_workflow_requires_native_screenshot_and_probe(self) -> None:
        ...
        self.assertIn("GetWindowThreadProcessId", capture_script)
        self.assertIn("IsOwnedWindow", capture_script)
        self.assertIn("PrintWindow", capture_script)
        self.assertIn("PW_RENDERFULLCONTENT", capture_script)
        self.assertIn("$process.Kill($true)", capture_script)
        self.assertNotIn("taskkill", capture_script)
        self.assertIn("$process.WaitForExit(10000)", capture_script)
