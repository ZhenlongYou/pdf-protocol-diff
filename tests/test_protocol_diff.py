...
"""
    ...
    def test_cross_card_citation_cleanup_keeps_technical_title_case_change(self) -> None:
        """引用扩展不得借位置折叠吞掉章节标题中的技术大小写变化。"""
        ...
        result = compare_extractions(
            ExtractionResult(
                pdf_path=Path("old_cross_card_title_case.pdf"),
                pages=[
                    PageText(
                        page_number=1,
                        text=f"31.3.17.2 CMIS-LT Profile\n{old_sentence}",
                    )
                ],
            ),
            ExtractionResult(
                pdf_path=Path("new_cross_card_title_case.pdf"),
                pages=[
                    PageText(
                        page_number=1,
                        text=f"31.3.18.2 cmis-lt Profile\n{new_sentence}",
                    )
                ],
            ),
            DiffOptions(),
    ...
    def test_cross_card_citation_cleanup_does_not_cross_same_parent_siblings(self) -> None:
        """同一父章下同时存在的同名兄弟章节也不得互借引用 occurrence。"""
        ...
        result = compare_extractions(
            ExtractionResult(
                pdf_path=Path("old_same_parent_siblings.pdf"),
                pages=[
                    PageText(
                        page_number=1,
                        text=(
            ...
            ExtractionResult(
                pdf_path=Path("new_same_parent_siblings.pdf"),
                pages=[
                    PageText(
                        page_number=1,
                        text=(
    ...
    def test_cross_card_citation_cleanup_is_disabled_for_partial_page_ranges(self) -> None:
        """局部页范围无法证明location唯一，跨卡引用必须保守显示。"""
        ...
        result = compare_extractions(
            ExtractionResult(
                pdf_path=Path("old_partial_range.pdf"),
                pages=[PageText(page_number=2, text=f"1 Scope\n{old_sentence}")],
                total_pages=10,
                selected_start_page=2,
                selected_end_page=2,
            ),
            ExtractionResult(
                pdf_path=Path("new_partial_range.pdf"),
                pages=[PageText(page_number=8, text=f"1 Scope\n{new_sentence}")],
                total_pages=10,
                selected_start_page=8,
                selected_end_page=8,
            ),
            DiffOptions(),
        )
    ...
    def test_schema_drift_cannot_reenable_repeated_table_pairing(self) -> None:
        """Repeated Parameter/Column identities remain unpaired across drift."""
        ...
        old_rows = [
            "表格行: T1 | Parameter=Input voltage | Condition=PAM4 | Value=20 mV",
            "表格行: T1 | Parameter=Input voltage | Condition=NRZ | Value=30 mV",
        ]
        new_rows = [
            "表格行: T1 | Column 1=Input voltage | Condition=NRZ | Value=31 mV",
            "表格行: T1 | Column 1=Input voltage | Condition=PAM4 | Value=21 mV",
        ]
        changes = reporting_module._table_row_changes(
            (table(old_rows),),
            (table(new_rows),),
    ...
    def test_proven_generic_column_boundary_reflow_is_not_a_change(self) -> None:
        """Two body rows may prove one consistent generic column merge."""
        ...
        old_rows = [
            "表格行: T1 | Column 1=Mode | Column 2=Type | Column 3=Value",
            "表格行: T1 | Column 1=Near | Column 2=end | Column 3=20",
            "表格行: T1 | Column 1=Far | Column 2=end | Column 3=30",
        ]
        new_rows = [
            "表格行: T1 | Column 1=Mode Type | Column 2=Value",
            "表格行: T1 | Column 1=Near end | Column 2=20",
            "表格行: T1 | Column 1=Far end | Column 2=30",
        ]
    ...
    def test_generic_column_reflow_does_not_hide_row_reordering(self) -> None:
        """Column-boundary evidence cannot consume rows in a new order."""
        ...
        old_rows = [
            "表格行: T1 | Column 1=Mode | Column 2=Type | Column 3=Value",
            "表格行: T1 | Column 1=First | Column 2=step | Column 3=20",
            "表格行: T1 | Column 1=Second | Column 2=step | Column 3=30",
        ]
        new_rows = [
            "表格行: T1 | Column 1=Mode Type | Column 2=Value",
            "表格行: T1 | Column 1=Second step | Column 2=30",
            "表格行: T1 | Column 1=First step | Column 2=20",
        ]
        changes = reporting_module._table_row_changes(
            (table(old_rows),),
            (table(new_rows),),
    ...
    def test_overwide_order_preview_distinguishes_later_columns(self) -> None:
        """Rows sharing Column 1 expose a distinguishing later cell."""

        def wide_row(mode: str) -> str:
            values = ["SAME_PRIORITY", mode, *(f"V{index}" for index in range(2, 33))]
            return "表格行: T1 | " + " | ".join(
                f"Column {index + 1}={value}"
                for index, value in enumerate(values)
            )
    ...
    def test_overwide_row_crossing_modified_explicit_anchor_is_visible(self) -> None:
        ...
        old_explicit = "表格行: T1 | Parameter=Receiver limit | Value=5 V"
        new_explicit = "表格行: T1 | Parameter=Receiver limit | Value=6 V"
        ...
        changes = reporting_module._table_row_changes(
            (table([wide, old_explicit]),),
            (table([new_explicit, wide]),),
        )
        evidence = "\n".join(
            f"{row.old_value}\n{row.new_value}" for row in changes
        )
        self.assertIn("5 V", evidence)
    ...
    def test_large_wide_count_shift_is_not_reordering(self) -> None:
        """Linear order fallback projects both sides to common occurrences."""
        ...
        beta = wide_row("B")
        changes = reporting_module._table_row_changes(
            (table([*([alpha] * 2001), *([beta] * 2000)]),),
            (table([*([alpha] * 2000), *([beta] * 2001)]),),
        )

        self.assertEqual(2, len(changes))
        self.assertFalse(any(row.change_type == "顺序变化" for row in changes))
        self.assertEqual(1, sum(row.change_type == "旧表删除行" for row in changes))
        self.assertEqual(1, sum(row.change_type == "新表新增行" for row in changes))
